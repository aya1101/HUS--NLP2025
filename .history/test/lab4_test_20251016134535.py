...
def main():
    ...
    with Tee(log_path):
        ...
        # 1) Vector cho 'king' — print full vector (JSON) so it goes into log4.txt
        try:
            king_vec = we.get_vector("king")
            import json
            print("\n=== KING_VECTOR_BEGIN ===")
            print(json.dumps(np.asarray(king_vec).tolist(), ensure_ascii=False))
            print("=== KING_VECTOR_END ===\n")
        ...
        try:
            ...
            # collect words and vectors and print them (JSON) so they land in log4.txt
            words = [w for w, _ in top]
            scores = [s for _, s in top]
            vectors = [we.get_vector(w) for w in words]
            import json
            print("\n=== COMPUTER_TOP10_BEGIN ===")
            for w, s, v in zip(words, scores, vectors):
                print(json.dumps({"word": w, "score": float(s), "vector": np.asarray(v).tolist()}, ensure_ascii=False))
            print("=== COMPUTER_TOP10_END ===\n")
        except Exception as e:
        ...
        try:
            ...
            import json
            print("\n=== DOC_VECTOR_BEGIN ===")
            print(json.dumps(np.asarray(doc_vec).tolist(), ensure_ascii=False))
            print("=== DOC_VECTOR_END ===\n")
