...
def vetor_search(spark, text, transformed_df):
    """Tìm kiếm văn bản tương tự trong tập dữ liệu đã chuyển đổi"""

    # Load lại mô hình pipeline đã fit
    pipeline_model = PipelineModel.load("results/lab3_pipeline_model")
    ...
    query_vector = query_transformed.select("normalized_features").first()[0]

    # Tìm kiếm văn bản tương tự trong tập dữ liệu đã chuyển đổi
    results = transformed_df \
        .select("text", "normalized_features") \
        .withColumn("similarity", F.expr(f"cosine_similarity(normalized_features, array({query_vector}))")) \
...
def main():
    ...
    try:
        ...
        # Tìm kiếm văn bản bằng vector search
        text = input("Input text: ")
        search_results = vetor_search(spark, text, transformed_df)
        print("\nSearch Results:")
        for result in search_results:
