...
def save_pipeline_results(transformed_df, start_time, end_time, fit_duration, transform_duration, vocab_duration, actual_vocab_size, search_results=None, search_duration=0, query_text=""):
    """Save pipeline results and log information according to Lab 17 requirements with vector search"""
    ...
    print("Saving feature vectors and search results to results/lab17_pipeline_output.txt...")
    
    output_file = "results/lab17_pipeline_output.txt"
    with open(output_file, "w", encoding="utf-8") as f:
        f.write("LAB 17 - SPARK ML PIPELINE OUTPUT WITH VECTOR SEARCH\n")
        ...
        if search_results and query_text:
            ...
            for i, result in enumerate(search_results, 1):
                ...
                f.write(f"Text: {result['text'][:150]}{'...' if len(result['text']) > 150 else ''}\n")
                f.write("-" * 30 + "\n")
            f.write("\n")
    ...
    with open(log_file, "w", encoding="utf-8") as f:
        f.write("LAB 17 - SPARK ML PIPELINE PROCESS LOG WITH VECTOR SEARCH\n")
        f.write("=" * 80 + "\n\n")
        ...
        if search_results:
            f.write(f"Vector search performed: Yes\n")
            f.write(f"Search query: {query_text}\n")
            f.write(f"Top results found: {len(search_results)}\n")
        else:
            f.write(f"Vector search performed: No\n")
        f.write("\n")
...
def main():
    ...
    try:
        ...
        print(f"   Vector search query: '{query_text}'")
