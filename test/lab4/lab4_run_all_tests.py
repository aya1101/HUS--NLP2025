...
def main():
    with Tee(LOG_FILE):
        ...
        print("\n\n" + "=" * 60)
        print("TEST 2: GENSIM LOCAL TRAINING (C4 DATASET)")
        print("=" * 60 + "\n")
        run_test(ROOT / 'test' / 'run_word2vec_tests.py')
        
        print("\n\n" + "=" * 60)
        print("TEST 3: PYSPARK TRAINING (C4 DATASET)")
        print("=" * 60 + "\n")
        run_test(
            ROOT / 'src' / 'spark' / 'lab4_task4_pyspark.py',
            ['--input', 'data/c4-train.00000-of-01024-30K.json', '--field', 'text', 
             '--vectorSize', '100', '--minCount', '5', '--no-save']
        )
        
        print("\n\n" + "=" * 60)
        print("TEST 4: EMBEDDING VISUALIZATION (2D & 3D)")
        print("=" * 60 + "\n")
        run_test(ROOT / 'test' / 'visualize_embeddings.py')
        print()
        run_test(ROOT / 'test' / 'visualize_trained_embeddings.py')
